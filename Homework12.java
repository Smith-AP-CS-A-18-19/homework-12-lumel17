...
public class Homework12 {
	...
	public int problem3() {
		int num = 0;
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i) instanceof Ellipse2D) {
				num++;
			}
		}
		return num;
	...
	public boolean problem4(Line2D l) {
		boolean check = false;
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i) instanceof Rectangle2D) {
					if (((Rectangle2D) (list.get(i))).intersectsLine(l)) {
						check = true;
					}
				}
			}
	...
	public double problem5() {
		double area = 0;
			for(int i = 0; i < list.size(); i++) {
				RectangularShape shape = list.get(i);
				if(list.get(i) instanceof Rectangle2D) {
					area += (shape.getHeight() * shape.getWidth());
				} else if(list.get(i) instanceof Ellipse2D) {
